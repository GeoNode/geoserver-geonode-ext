...
public class FreemarkerModelConverterTest {
    ...
    public void testJSON() throws IOException {
        ...
        PrintControllerTemplateWriterTest.createTemplate(
                "farby.ftl",
                new StringReader(
                        "${title}"
        ...
        MockHttpInputMessage req =
                new MockHttpInputMessage(new ByteArrayInputStream(obj.toString().getBytes()));
        req.getHeaders().setContentType(MediaType.APPLICATION_JSON);
        SimpleHash simpleHash = converter.readInternal(null, req);

        String text =
                new Scanner(PrintController.writeTemplate("farby", simpleHash))
                        .useDelimiter("\\Z")
                        .next();
