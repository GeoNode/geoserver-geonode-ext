...
abstract class HTMLMapLayer {
    ...
    abstract List<Tile> getTiles(
            int width, int height, String srs, Envelope bounds, double resolution);
    ...
    static class WMSLayer extends HTMLMapLayer {
        ...
        List<Tile> getTiles(int width, int height, String srs, Envelope bounds, double resolution) {
            return Collections.singletonList(
                    new Tile(new Point(0, 0), new Dimension(width, height), bounds, srs, this));
        }
    }

    abstract static class Grid extends HTMLMapLayer {
        ...
        List<Tile> getTiles(int width, int height, String srs, Envelope bounds, double resolution) {
            ...
            int minRows =
                    (int) Math.ceil((double) viewSize.height / (double) this.tileSize.height)
                            + Math.max(1, 2 * this.buffer);
            int minCols =
                    (int) Math.ceil((double) viewSize.width / (double) this.tileSize.width)
                            + Math.max(1, 2 * this.buffer);
