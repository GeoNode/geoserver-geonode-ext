...
public class PrintController extends RestBaseController {
    ...
    static class PrintControllerConfiguration {
        ...
        ContentNegotiationStrategy printOutputTypeContentNegotiationStrategy() {
            return webRequest -> {
                ...
                if (new PatternsRequestCondition(
                                        "/printng/render.{ext}",
                                        "/printng/freemarker/{template}.{ext}")
    ...
    @PostMapping(
        ...
        produces = {
            MediaType.APPLICATION_PDF_VALUE, MediaType.IMAGE_GIF_VALUE, MediaType.IMAGE_JPEG_VALUE,
            MediaType.IMAGE_PNG_VALUE, MediaType.APPLICATION_JSON_VALUE, MediaType.TEXT_HTML_VALUE
        }
    )
    public PrintSpec renderHtmlPost(
            InputStream inputStream, @RequestParam MultiValueMap<String, String> params)
            throws IOException {
    ...
    public PrintSpec renderFreemarkerPost(
            @RequestBody SimpleHash simpleHash,
    ...
    protected static String writeTemplate(String templateName, SimpleHash model)
            throws IOException {
    ...
    private static PrintSpec getPrintSpec(Reader reader, MultiValueMap<String, String> queryParams)
            throws IOException {
    ...
    public String modifyFreemarkerPost(
            InputStream input, @PathVariable(name = "template") String templateName) {
        if (!templateName.endsWith(".ftl")) {
        ...
        try (InputStreamReader reader = new InputStreamReader(input)) {
            ...
            try (Writer writer = GeoserverSupport.newTemplateWriter(templateName)) {
                ...
                throw new RestException(
                        "Error writing new template", HttpStatus.INTERNAL_SERVER_ERROR, e);
            }
        } catch (IOException e) {
            throw new RestException("Error parsing invalid input", HttpStatus.BAD_REQUEST, e);
        }
    }
}
